...
/* #define USE_DQ_TIMER */
#ifdef USE_DQ_TIMER
#define DQ_TIMEOUT_MS		(200)
/*#define DQ_TIMEOUT_MS		(30)*/
#include <linux/timer.h>
#endif
...
#ifdef USE_DQ_TIMER
static void handle_dq_timeout(struct timer_list *priv)
{
	struct nx_clipper *me =
		container_of(priv, struct nx_clipper, dq_timer);

	pr_err("[CLI %d] Timeout\n", me->module);
...
#ifdef USE_DQ_TIMER
			timer_setup((struct timer_list*)&me->dq_timer, handle_dq_timeout,
					(struct timer_list*)&me->dq_timer);
#endif
				 ...
				 u32 flags)
{
	...
	pr_debug("[%s] entity: local:%d:%s:%d, remote:%d:%s:%d\n", __func__, local->index,
			local->entity->name, media_entity_type(local->entity),
			remote->index,
			remote->entity->name,
			media_entity_type(remote->entity));

	if (local->index == NX_CLIPPER_PAD_SOURCE_MEM &&
			media_entity_type(remote->entity) == MEDIA_ENTITY_TYPE_VIDEO_DEVICE) {
...
static int init_v4l2_subdev(struct nx_clipper *me)
{
	...
	ret = media_entity_pads_init(entity, NX_CLIPPER_PAD_MAX, pads);
	if (ret < 0) {
				    ...
				    struct v4l2_subdev *sd)
{
	...
	if (!list_empty(&sd->entity.links)) {
		...
		return media_entity_pads_init(&sd->entity, 1, pad);
	}
