...
static int add_config(struct device *dev, unsigned long **configs,
		      unsigned int *num_configs, unsigned long config)
{
...
static void nexell_pinmux_setup(struct pinctrl_dev *pctldev,
...
static int nexell_pinmux_set_mux(struct pinctrl_dev *pctldev,
				 unsigned int selector,
				 unsigned int group)
{
			     ...
			     unsigned long *config, bool set)
{
	...
	if (set) {
		...
		ret = nexell_soc_write_pin(io, cfg_type, cfg_value);
		if (ret < 0)
			return ret;
	} else {
		ret = nexell_soc_read_pin(io, cfg_type, &cfg_value);
		if (ret < 0)
			return ret;
				    ...
				    unsigned int num_configs)
{
	...
	for (cnt = 0; cnt < drvdata->pin_groups[group].num_pins; cnt++) {
		ret = nexell_pinconf_set(pctldev, pins[cnt],
				configs, num_configs);
		if (ret < 0)
			break;
	}
				    ...
				    unsigned int group, unsigned long *config)
{
	...
	drvdata = pinctrl_dev_get_drvdata(pctldev);
	pins = drvdata->pin_groups[group].pins;
	ret = nexell_pinconf_get(pctldev, pins[0], config);
	if (ret < 0)
		return ret;
	return 0;
}
...
static void nx_gpio_set(struct gpio_chip *gc, unsigned int offset, int value)
{
...
static int nx_gpio_get(struct gpio_chip *gc, unsigned int offset)
{
	u32 data;
	struct nexell_pin_bank *bank = gc_to_pin_bank(gc);
	int io;
 ...
 */
static int nx_gpio_set_direction(struct gpio_chip *gc, unsigned int offset,
				 bool input)
{
	struct nexell_pin_bank *bank;
	struct nexell_pinctrl_drv_data *drvdata;
...
static int nx_gpio_direction_input(struct gpio_chip *gc, unsigned int offset)
{
	return nx_gpio_set_direction(gc, offset, true);
}

/* gpiolib gpio_direction_output callback function. */
static int nx_gpio_direction_output(struct gpio_chip *gc, unsigned int offset,
				    int value)
{
 ...
 */
static int nx_gpio_to_irq(struct gpio_chip *gc, unsigned int offset)
{
	struct nexell_pin_bank *bank = gc_to_pin_bank(gc);
	unsigned int virq;
...
static int nx_gpio_request(struct gpio_chip *gc, unsigned int offset)
{
	struct nexell_pin_bank *bank;
	struct nexell_pinctrl_drv_data *drvdata;
...
static void nx_gpio_free(struct gpio_chip *gc, unsigned int offset)
{
...
static int nexell_pinctrl_probe(struct platform_device *pdev)
{
	...
	if (i != ctrl->nr_banks) {
		dev_err(&pdev->dev, "bank count mismatch(%d/%d)\n", i, ctrl->nr_banks);
		return -EINVAL;
	}
