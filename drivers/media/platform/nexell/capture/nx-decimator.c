...
static int alloc_dma_buffer(struct nx_decimator *me)
{
	if (me->buf.addr == NULL) {
		...
		if (me->buf.format == MEDIA_BUS_FMT_YVYU12_1X24) {
		...
		} else if (me->buf.format == MEDIA_BUS_FMT_YVYU12_1X24) {
			...
			y_size = me->buf.stride[0] * ALIGN(me->height, 16);
			cbcr_size = 0;
		}
...
static void handle_dq_timeout(struct timer_list *priv)
{
	struct nx_decimator *me =
		container_of(priv, struct nx_decimator, dq_timer);

	pr_err("[DEC %d] Timeout\n", me->module);
...
#ifdef USE_DQ_TIMER
			timer_setup(&me->dq_timer, handle_dq_timeout,
				 ...
				 u32 flags)
{
	...
	if (local->index == NX_DECIMATOR_PAD_SOURCE_MEM &&
