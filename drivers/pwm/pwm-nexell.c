...
const char *pwm_tclk_name[PWM_NUM + 1];
...
static int pwm_nexell_parse_dt(struct nexell_pwm_chip *nx_pwm)
{
	...
	of_property_for_each_string(np, "clock-names", prop, tclk_name) {
		if (n >= 5)
			break;
		pwm_tclk_name[n++] = tclk_name;
	}
...
static int pwm_nexell_probe(struct platform_device *pdev)
{
	...
	nx_pwm->apbclk = devm_clk_get(dev, pwm_tclk_name[0]);
	...
	for (ch = 0; ch < PWM_NUM; ++ch) {
		nx_pwm->tclk[ch] = devm_clk_get(dev, pwm_tclk_name[ch + 1]);
