...
{
	switch (media_entity_type(remote->entity)) {
				 ...
				 struct device *vb2_alloc_dev)
{
	...
	me->vb2_alloc_dev = vb2_alloc_dev;
	me->pads[0].flags = MEDIA_PAD_FL_SINK;
	me->pads[1].flags = MEDIA_PAD_FL_SOURCE;
	pad_num = 2;

	me->vdev.entity.ops = &nx_media_entity_operations;
	me->vdev.entity.obj_type = MEDIA_ENTITY_TYPE_VIDEO_DEVICE;
	me->vdev.entity.function = MEDIA_ENT_F_IO_V4L;
	ret = media_entity_pads_init(&me->vdev.entity, pad_num, me->pads);
	if (ret < 0) {
		kfree(me);
...
void nx_video_cleanup(struct nx_video *me)
{
	...
	media_entity_remove_links(&me->vdev.entity);
	if (me->vbq) {
		vb2_queue_release(me->vbq);
		kfree(me->vbq);
	}
